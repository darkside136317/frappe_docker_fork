  ...
  <div class="container mx-auto" v-if="this.menu.paginatedItems.length > 0">
    <div class="mt-4 grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
      ...
      >
        ...
        <p
          v-if="menuStockLineVisible(item)"
          class="mt-1 text-center text-xs text-gray-500"
        >
          Available:
          {{
            Number(menuStockQty(item)).toLocaleString(undefined, {
        ...
        <div v-if="!item.qty" class="text-center">
          <button
            :disabled="!this.menu.itemIsOrderable(item)"
            @click="
              item.showInput = true;
              this.menu.addToCart(item);
            "
            class="mt-2 rounded border px-10 pb-2 pt-2.5 text-xs font-medium leading-normal disabled:cursor-not-allowed disabled:opacity-50"
          >
            ADD +
          </button>
        </div>
        <div v-if="item.qty" class="mt-2 flex rounded-md text-center">
          ...
          <button
            type="button"
            class="-ml-px inline-flex items-center justify-center gap-2 border bg-white px-4 py-3 align-middle text-sm font-medium text-gray-700 shadow-sm transition-all focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700"
            :disabled="
              this.recentOrders.restaurantTable ||
              !this.menu.itemCanIncrementQuantity(item)
            "
...
export default {
  ...
  methods: {
    menuStockLineVisible(item) {
      const meta = this.menu.findMenuMetaByItemCode(item.item) || item;
      return this.menu.shouldEnforceStock(meta);
    },
    menuStockQty(item) {
      const meta = this.menu.findMenuMetaByItemCode(item.item) || item;
      return this.menu.getAvailableStockQty(meta);
    },
  },
};
</script>
