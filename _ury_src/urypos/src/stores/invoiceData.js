...
export const useInvoiceDataStore = defineStore("invoiceData", {
  ...
  actions: {
    ...
    async invoiceCreation() {
      ...
      const cartStock = this.menu.validateCartStock();
      if (!cartStock.ok) {
        await this.alert.createAlert("Insufficient stock", cartStock.message, "OK");
        this.showUpdateButtton = true;
        this.invoiceUpdating = false;
        return;
      }
        ...
        if (response.message.status === "Failure") {
          if (response._server_messages) {
            ...
            await this.alert.createAlert(
              "Message",
              "Order update failed. Please reload and try again.",
              "OK"
            );
          }
