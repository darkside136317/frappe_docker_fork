...
export const useMenuStore = defineStore("menu", {
  ...
  actions: {
    ...
    findMenuMetaByItemCode(itemCode) {
      if (!itemCode) return null;
      return this.items.find((m) => m.item === itemCode) || null;
    },
    shouldEnforceStock(meta) {
      if (!meta) return false;
      const hasAvail =
        meta.available_qty !== undefined &&
        meta.available_qty !== null &&
        !Number.isNaN(Number(meta.available_qty));
      const hasQty =
        meta.stock_qty !== undefined &&
        meta.stock_qty !== null &&
        !Number.isNaN(Number(meta.stock_qty));
      if (hasAvail || hasQty) return true;
      return Boolean(Number(meta.is_stock_item));
    },
    getAvailableStockQty(meta) {
      if (!meta) return Number.POSITIVE_INFINITY;
      if (!this.shouldEnforceStock(meta)) return Number.POSITIVE_INFINITY;
      if (
        meta.available_qty !== undefined &&
        meta.available_qty !== null &&
        !Number.isNaN(Number(meta.available_qty))
      ) {
        return Number(meta.available_qty);
      }
      if (meta.stock_qty === undefined || meta.stock_qty === null) return 0;
      return Number(meta.stock_qty);
    },
    itemIsOrderable(item) {
      const meta = this.findMenuMetaByItemCode(item.item) || item;
      if (!this.shouldEnforceStock(meta)) return true;
      return this.getAvailableStockQty(meta) > 0;
    },
    itemCanIncrementQuantity(item) {
      const meta = this.findMenuMetaByItemCode(item.item) || item;
      if (!this.shouldEnforceStock(meta)) return true;
      const avail = this.getAvailableStockQty(meta);
      const cartLine = this.cart.find((c) => c.item === item.item);
      const inCart = cartLine ? Number(cartLine.qty) || 0 : 0;
      return inCart < avail;
    },
    validateCartStock() {
      for (const line of this.cart) {
        const meta = this.findMenuMetaByItemCode(line.item);
        if (!this.shouldEnforceStock(meta)) continue;
        const avail = this.getAvailableStockQty(meta);
        const need = Number(line.qty);
        if (need > avail) {
          const name = line.item_name || (meta && meta.item_name) || line.item;
          return {
            ok: false,
            message: `Insufficient stock for ${name}: available ${avail}, ordered ${need}.`,
          };
        }
      }
      return { ok: true };
    ...
    addToCartAndUpdateQty() {
      ...
      if (
        ...
        if (this.shouldEnforceStock(meta) && qty > this.getAvailableStockQty(meta)) {
          const name = item.item_name || item.item;
          const avail = this.getAvailableStockQty(meta);
          this.alert.createAlert(
            "Insufficient stock",
            `Maximum available is ${avail} for ${name}.`,
            "OK"
          );
          return;
        }
    ...
    addToCart(item) {
      if (!this.itemIsOrderable(item)) {
        const meta = this.findMenuMetaByItemCode(item.item) || item;
        const name = item.item_name || item.item;
        const avail = this.getAvailableStockQty(meta);
        this.alert.createAlert(
          "Insufficient stock",
          `Cannot add ${name}: out of stock (available ${avail}).`,
          "OK"
        );
        return;
      }
    ...
    incrementItemQuantity(item) {
      ...
      const itemIndexExists = itemIndex !== -1;
      const meta = this.findMenuMetaByItemCode(item.item) || item;
      if (this.shouldEnforceStock(meta)) {
        const avail = this.getAvailableStockQty(meta);
        const inCart = itemIndexExists ? Number(this.cart[itemIndex].qty) || 0 : 0;
        if (inCart + 1 > avail) {
          this.alert.createAlert(
            "Insufficient stock",
            `Cannot add more than ${avail} for ${item.item_name || item.item}.`,
            "OK"
          );
          return;
        }
