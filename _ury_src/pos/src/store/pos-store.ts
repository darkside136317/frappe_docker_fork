...
import { syncCurrencyStorageFromProfile } from '../lib/sync-currency-storage';
...
export interface MenuItem extends Omit<APIMenuItem, 'rate' | 'item_image'> {
  ...
  image: string | null;
  price: number;
  /** On-hand in POS warehouse when loaded from getRestaurantMenu; null when not a stock item */
  stock_qty?: number | null;
  /** Same as API: physical minus draft POS reservations */
  available_qty?: number | null;
  /** ERPNext Item.is_stock_item (0/1); when 1, cart qty cannot exceed available / stock */
...
interface POSStore extends POSState {
  ...
  validateActiveOrdersStock: () => { ok: true } | { ok: false; message: string };
...
export const usePOSStore = create<POSStore>((set, get) => ({
  ...
  initializeApp: async () => {
    try {
      ...
      const [menuResult, categoriesResult, paymentModesResult] = await Promise.allSettled([
        get().fetchMenuItems(),
        get().fetchCategories(),
  ...
  fetchPosProfile: async () => {
    try {
      ...
      if (cached) {
        ...
        await syncCurrencyStorageFromProfile(profile);
        return;
      }
      ...
      await syncCurrencyStorageFromProfile(combinedProfile);
    } catch (error) {
  ...
  fetchCurrencySymbol: async () => {
    try {
      ...
      const symbol = (response.symbol || response.name || currency || '').trim();
      if (symbol && symbol !== 'undefined' && symbol !== 'null') {
        set({ currencySymbol: symbol });
        storage.setItem('currencySymbol', symbol);
      } else {
        set({ currencySymbol: currency });
        storage.setItem('currencySymbol', currency);
      }
    } catch (error) {
      console.error('Error fetching currency symbol:', error);
      const c = get().currency || 'INR';
      set({ currencySymbol: c });
      storage.setItem('currencySymbol', c);
    }
  },
  ...
  fetchMenuItems: async () => {
    ...
    try {
      ...
      const menuItems: MenuItem[] = items.map(item => ({
        ...
        tax_rate: 0,
        stock_qty: item.stock_qty !== undefined && item.stock_qty !== null ? Number(item.stock_qty) : undefined,
        available_qty:
          item.available_qty !== undefined && item.available_qty !== null
            ? Number(item.available_qty)
            : undefined,
        is_stock_item:
          item.is_stock_item !== undefined && item.is_stock_item !== null
            ? Number(item.is_stock_item)
            : undefined,
      }));
  ...
  addToOrder: async (item: OrderItem) => {
    try {
      ...
      const itemCode = itemCodeForStock(item);
      const menuLine = get().menuItems.find((m) => (m.item || m.id) === itemCode);

      if (shouldEnforceStockFromMenuLine(menuLine)) {
        const avail = stockAvailable(menuLine);
        const orders = get().activeOrders;
        const mergedQty =
          existingItemIndex !== -1
            ? orders[existingItemIndex].quantity + item.quantity
            : item.quantity;
        const others = orders.reduce((sum, o, idx) => {
          if (idx === existingItemIndex) return sum;
          if (itemCodeForStock(o) !== itemCode) return sum;
  ...
  updateQuantity: async (uniqueId: string, quantity: number) => {
    try {
      ...
      const orders = get().activeOrders;
      const lineIndex = orders.findIndex((o) => o.uniqueId === uniqueId);
      if (lineIndex === -1) return;

      const itemCode = itemCodeForStock(orders[lineIndex]);
      const menuLine = get().menuItems.find((m) => (m.item || m.id) === itemCode);
  ...
  validateActiveOrdersStock: () => {
    const r = validateActiveOrdersAgainstMenu(get().menuItems, get().activeOrders);
    if (r.ok) return { ok: true };
    return {
      ok: false,
      message: t('errors.insufficient_stock_order', {
        name: r.name,
