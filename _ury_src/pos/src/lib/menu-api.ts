...
export interface MenuItem {
  ...
  description?: string;
  special_dish?: 1 | 0;
  /** On-hand qty in POS warehouse; null when Item is not a stock item */
  stock_qty?: number | null;
  /** Qty left to sell given draft POS orders (same warehouse); null when not a stock item */
  available_qty?: number | null;
  /** From Item.is_stock_item — stock rules apply only when 1 */
