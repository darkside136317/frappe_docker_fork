...
const ProductDialog: React.FC<ProductDialogProps> = ({
  ...
  const qtySameItemOtherLines = useMemo(() => {
    if (!selectedItem) return 0;
    const code = selectedItem.item || selectedItem.id;
    return activeOrders.reduce((sum, o) => {
      if (editMode && itemToReplace && o.uniqueId === itemToReplace.uniqueId) return sum;
      if ((o.item || o.id) !== code) return sum;
      return sum + o.quantity;
    }, 0);
  }, [selectedItem, activeOrders, editMode, itemToReplace]);

  const maxOrderableQty = useMemo(() => {
    if (!selectedItem || !shouldEnforceStockFromMenuLine(selectedItem)) return 99;
    const cap = stockAvailable(selectedItem);
    if (cap === Number.POSITIVE_INFINITY) return 99;
    return Math.max(0, Math.min(99, cap - qtySameItemOtherLines));
  }, [selectedItem, qtySameItemOtherLines]);
  ...
  const handleQuantityChange = (value: string) => {
    ...
    if (!isNaN(num) && num >= 0 && num <= maxOrderableQty) {
            ...
            <p className="text-sm text-gray-600 mt-2">
              {shouldEnforceStockFromMenuLine(selectedItem) ? (
                ...
                <>
                  ...
                  {typeof selectedItem?.stock_qty === 'number' &&
                  !Number.isNaN(selectedItem.stock_qty)
                    ? Number(selectedItem.stock_qty).toLocaleString(undefined, {
                        maximumFractionDigits: 2,
                      })
                    : '—'}
                </>
        ...
        <div className="h-auto md:w-1/3 p-6 border-t md:border-t-0 md:border-l border-gray-200 overflow-y-auto flex flex-col">
          ...
          <div className="mt-auto pt-2 border-t border-gray-200">
            ...
            <Button
              ...
              size="lg"
              disabled={numericQuantity === 0 || maxOrderableQty < 1 || numericQuantity > maxOrderableQty}
            >
