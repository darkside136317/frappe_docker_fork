...
from ury.ury_pos.api import getBranch, getBranchRoom, validate_pos_order_items_stock
...
def sync_order(
    ...
    # Keep warehouse explicit so draft reservation SQL can include this invoice reliably.
    invoice.set_warehouse = posprofile.warehouse
    ...
    branch_for_stock = getattr(invoice, "branch", None) or getBranch()
    validate_pos_order_items_stock(
        items,
        posprofile,
        exclude_invoice_name=invoice.name if getattr(invoice, "name", None) else None,
        branch=branch_for_stock,
    )
...
def cancel_order(invoice_id, reason):
    ...
    # Use canonical ERPNext cancel flow when already submitted.
    if pos_invoice.docstatus == 1:
        try:
            pos_invoice.cancel()
        except Exception as e:
            frappe.throw(_("Error while cancelling submitted invoice: {0}").format(e))
...
def make_invoice(customer, payments, cashier, pos_profile,owner, additionalDiscount=None, table=None, invoice=None):
    ...
    order_items = [
        {"item": row.item_code, "qty": row.qty} for row in invoice.items
    ]
    validate_pos_order_items_stock(
        order_items,
        posprofile,
        exclude_invoice_name=invoice.name,
        branch=getattr(invoice, "branch", None) or getBranch(),
    )
